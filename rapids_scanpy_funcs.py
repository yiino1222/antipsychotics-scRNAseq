...
def preprocess_in_batches(input_file, markers, min_genes_per_cell=200, max_genes_per_cell=6000, 
    ...
    with h5py.File(input_file, 'r') as h5f:
        indptrs = h5f[_indptr]
        #genes = cudf.Series(h5f[_genes], dtype=cp.dtype('object'))
        genes = cudf.Series(h5f[_genes], dtype=cp.dtype('object')).str.upper()
        if max_cells is not None:
    ...
    #print(f"number of gene : {len(genes)}")
    #print(f"shape of gene counts : {gene_counts.shape}")
    ...
    for batch_start in range(0, n_cells, cell_batch_size):
       ...
       # print(f"shape of partial_sparse_array: {partial_sparse_array.shape}")
       # print(f"indeces of partial_sparse_array: {partial_sparse_array.indices}")
    
        # Update gene count
        gene_counts += cp.bincount(partial_sparse_array.indices,minlength=len(genes))
    ...
    variable_genes = _cellranger_hvg(mean, mean_sq, genes_filtered.to_pandas(), n_cells_filtered, n_top_genes)
